...
"""
        ...
        try:
            ...
            output = json.loads(result.stdout)
            assert (
                output["enhancement_metadata"]["enhancement_level"] == "comprehensive"
            )
    ...
    def test_cli_argument_precedence_over_configuration(self):
        ...
        if (
            ...
            pytest.skip(
                "Configuration system not implemented yet - precedence testing needs config"
            )
...
"""
            ...
            try:
                ...
                assert (
                    result.returncode == 0
                ), f"Environment variable handling failed: {result.stderr}"
...
"""
    ...
    def test_help_text_shows_configuration_defaults(self):
        ...
        assert (
            "agents/brand_identity_generator/brand_identity_generator.py" in help_text
        )
        assert "--enhance" in help_text
        assert "--llm-provider" in help_text

        # Verify configuration defaults are surfaced in help text
        assert "LLM Provider: openai" in help_text
        assert "Enhancement Level: moderate" in help_text
        assert "Output Directory: ./output" in help_text
        assert "Debug Mode: False" in help_text
