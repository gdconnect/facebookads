...
class ConfigModel(BaseModel):
    ...
    model_enabled: bool = Field(default=False, description="Enable LLM model calls")
    model_provider: Literal["openai", "anthropic", "azure", "gemini"] = "openai"
    model_name: str = Field(default="gpt-4-turbo", description="LLM model identifier")
    model_temperature: float = Field(default=0.3, ge=0.0, le=2.0)
    model_max_tokens: int = Field(default=2000, ge=100, le=4000)
    model_timeout_seconds: int = Field(default=120, ge=30, le=300)

    # Performance Budgets (Constitutional Article XII)
    max_evaluation_time_seconds: int = Field(default=120, ge=60, le=300)
    max_cost_usd: float = Field(default=1.0, ge=0.1, le=5.0)
...
class MetaModel(BaseModel):
    ...
    cost: Dict[str, Union[int, float]] = Field(
...
"""
    ...
    def _identify_relative_strengths(
        ...
        if (
            idea.raw_idea.monetization and (
